...
let rows: TreeRow[] = [];
let stats: TreeStats = { collections: 0, requests: 0 };
let mascotUri = '';
...
function buildWelcome(): HTMLElement {
    ...
    if (mascotUri) {
        const mascot = el('img', 'welcome-mascot');

        mascot.src = mascotUri;
        mascot.alt = '';
        welcome.appendChild(mascot);
    }

    const heading = el('h1', 'welcome-title');

    heading.textContent = 'Welcome to Reqly';
    const text = el('p', 'welcome-text');

    text.textContent = 'Your collections and requests will appear here.';
    const actions = el('div', 'welcome-actions');
    const newRequest = el('button', 'welcome-btn primary');
    ...
    const newCollection = el('button', 'welcome-btn');
...
function buildFooter(): HTMLElement {
    ...
    const summary = el('span', 'footer-stats');

    if (stats.requests > 0 || stats.collections > 0) {
        const parts: string[] = [];
        ...
        parts.push(`${stats.requests} request${stats.requests === 1 ? '' : 's'}`);
        summary.textContent = parts.join(' · ');
    }

    const link = el('button', 'footer-link');

    link.type = 'button';
    link.title = 'Open reqly-labs on GitHub';
    link.appendChild(icon('github'));
    link.appendChild(document.createTextNode('reqly-labs'));
    link.addEventListener('click', () => post({ type: 'openRepository' }));
...
function render(): void {
    ...
    if (rows.length === 0) {
        root.append(buildWelcome(), buildFooter());
...
window.addEventListener('message', (event: MessageEvent<CollectionsHostMessage>) => {
    ...
    switch (message.type) {
        case 'render':
            rows = message.rows ?? [];
            stats = message.stats ?? { collections: 0, requests: 0 };
            mascotUri = message.mascotUri ?? '';
